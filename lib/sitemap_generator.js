...
module.exports = {
	...
	setup: function(config) {
		...
		self.priority = sitemap_config.priority || self.priority;
		...
		// extension
		self.urlExtension = sitemap_config.url_extension || "";
		self.useUrlExtension = self.urlExtension.length ? true : false;
	...
	createSitemap: function(callback) {
		...
		for (var i = 0; i < self.urls.length; i++) {
			...
			if (self.urls[i].changefreq === "false") {
				changefreq = "";
			} else {
				changefreq = "<changefreq>" + self.urls[i].changefreq + "</changefreq>";
			}

			if (self.urls[i].priority === "false") {
				priority = "";
			} else {
	...
	addUrl: function(file_path, callback) {
		...
		if (extname === ".html" || extname === "") {
			var file_portions = file_path.replace(/^\//, '').replace(/\\/g,"/").split("/");
			...
			var priority = self.priority || (1 - ( 0.1 * file_portions.length ));
			...
			// add the extension if an extension is defined or if there is no trailing slash
			if (self.useUrlExtension && !(new RegExp('[(' + self.urlExtension + ')/]$')).test(loc)) {
				loc += self.urlExtension;
			}
